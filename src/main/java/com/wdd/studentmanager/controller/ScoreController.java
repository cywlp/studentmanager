...
public class ScoreController {
    ...
    @GetMapping("/failSore_list")
    public String failSoreList(){
        return "/score/failScoreList";
    }
    ...
    @RequestMapping("/getFailScoreList")
    @ResponseBody
    public Object getFailScoreList(@RequestParam(value = "page", defaultValue = "1")Integer page,
                               @RequestParam(value = "rows", defaultValue = "100")Integer rows,
                               @RequestParam(value = "studentid", defaultValue = "0")String studentid,
                               @RequestParam(value = "courseid", defaultValue = "0")String courseid,
                               String from, HttpSession session){
        ...
        PageBean<Score> pageBean = scoreService.queryFailScorePage(paramMap);
        if(!StringUtils.isEmpty(from) && from.equals("combox")){
            return pageBean.getDatas();
        }else{
            Map<String,Object> result = new HashMap();
    ...
    @RequestMapping("/exportFailScore")
    @ResponseBody
    private void exportFailScore(HttpServletResponse response,Score score,HttpSession session) {
        //获取当前登录用户类型
        Student student = (Student) session.getAttribute(Const.STUDENT);
        if(!StringUtils.isEmpty(student)){
            //如果是学生，只能查看自己的信息
        ...
        try {
            response.setHeader("Content-Disposition", "attachment;filename="+ URLEncoder.encode("fail_score_list_sid_"+score.getStudentId()+"_cid_"+score.getStudentId()+".xls", "UTF-8"));
            response.setHeader("Connection", "close");
            response.setHeader("Content-Type", "application/octet-stream");
            ServletOutputStream outputStream = response.getOutputStream();
            List<Score> scoreList = scoreService.getFailScore(score);
            XSSFWorkbook xssfWorkbook = new XSSFWorkbook();
            XSSFSheet createSheet = xssfWorkbook.createSheet("成绩列表");
            XSSFRow createRow = createSheet.createRow(0);
            createRow.createCell(0).setCellValue("学生");
