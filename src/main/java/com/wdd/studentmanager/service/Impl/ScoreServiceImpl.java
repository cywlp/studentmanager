...
public class ScoreServiceImpl implements ScoreService {
    ...
    @Override
    public PageBean<Score> queryFailScorePage(Map<String, Object> paramMap) {
        PageBean<Score> pageBean = new PageBean<>((Integer) paramMap.get("pageno"),(Integer) paramMap.get("pagesize"));

        Integer startIndex = pageBean.getStartIndex();
        paramMap.put("startIndex",startIndex);
        List<Score> datas = scoreMapper.queryFailScoreList(paramMap);
        pageBean.setDatas(datas);

        Integer totalsize = scoreMapper.queryFailScoreCount(paramMap);
        pageBean.setTotalsize(totalsize);
        return pageBean;
    }
    ...
    @Override
    public List<Score> getFailScore(Score score) {
        return scoreMapper.getFail(score);
    }
